...
int	read_PC_array_for_matrix_mult(FILE *fPC, int program_memory[], struct registers *r , FILE *fnew)
{
	...
	fPC = fopen( "array_mult_assembly_PConly.c", "rt" );
...
int	read_instr_for_matrix_mult(FILE *finstr, int program_memory[], struct registers *r, FILE *fnew)
{
	...
	finstr = fopen( "array_mult_assembly_instruction_only.c", "rt" );
...
//      i1->reg_index = (i1->reg_file_addr) - 12; // Reg file starts only from 0CH = 12
        ...
        i1->reg_index = (i1->reg_file_addr); // This is the variable used in execute.h
